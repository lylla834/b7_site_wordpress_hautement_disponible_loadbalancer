#############################################
NomDuGroupe=Lyllaresgrp
NomDuVnet=vnetlylla
IpAddressVnet=10.0.0.0/16
NomDuSubnet=subnetlylla
IpAddressSubnet=10.0.0.0/24
NomIpPublic=monIPPubliclylla
NomDuLb=monLoadBalancerlylla
NomBackendPool=monBackendPoolylla
NomIpBackend=monIPbackendlylla
IpAddressBackend=ipadressBacklylla
NomHealthProbe=monHealthprobelylla
NomVm=maVMlylla
AdminName=lylla
AdminPwd=Jevis@ambronay
PrivateIp=10.0.0.5
FrontendIpName=monIPfrontlylla
NomHTTPRule=moHTTPrulelylla
NomNSG=monNSGlylla
NomNSGRuleHTTP=monNSGruleHTTPlylla
NomNic=monNiclylla
NomNatGateway=monNATgatelylla
NomMariadb=mariadatabaselylla
NomAdminMDB=lylla
Passadmin=Jevis@ambronay
nbzone=3
...
echo "Nom du groupe"
read NomDuGroupe
az group create --location westeurope --name $NomDuGroupe
...
Create_vnet(){
 ...
 az network vnet create -l westeurope -g $NomDuGroupe -n $NomDuVnet --address-prefix $IpAddressVnet --subnet-name $NomDuSubnet --subnet-prefix $IpAddressSubnet
